...
Yet, this paper introduces a divergent interpretation of the web's evolution. We argue that the essence of the next-generation web will be defined not by the redistribution of value, but by the democratisation of trust and the genesis of public goods.
...
[^rpc-replaces-restful]: The adaptation of Remote Procedure Calls (RPC) as a substitute for the traditional RESTful API of Web 2.0, was exemplified in our 2018 experiment with FIFA ticket tokenization, where tokenized FIFA tickets were issued to a select group of testers, including Vitalik Buterin. In this experiment, the location data for FIFA venues was retrieved from a peripheral smart contract, bypassing conventional web service routes, despite it has nothing to do with contractual logic. This methodology is still being used as an evolutionary stepping stone.
...
There are 2 places for Token Runtime Environment to exist.

1. A Token Runtime Environment can exist on a wallet, if majority of the token functions can be provided by smart contracts, and websites (Dapps) are interested in accessing Token functionality through wallet interfaces.
