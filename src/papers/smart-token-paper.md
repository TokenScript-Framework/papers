...
### Structuring the Web around origins rather than topics (like USENET) or functionality (like BitTorrent) would have unforseen consequences. It led to websites' single-origin design, reminiscent of how books have specific authors. Today, multi-domain sites are rare. This design choice profoundly shaped our trust paradigm: we interact with a site based on the trust we place in its origin. The website's credibility is often more important than the information it holds.

When Tim Berners-Lee and his team developed the foundational concepts of the Web they selected "sites" as its primary building blocks. This seemingly intuitive approach, however, was not a given; especially when other Internet protocols like emails and USENET and did not revolve around the concept of sites. Consider USENET: it organises and manages information by topic, making it irrelevant which site or even which planet the information originates from under that topic.
...
Recognising this, a decentralised trust anchor is not the service that makes the token interact with applications - the trust anchor is the token. By extention, a time-gated attestation signed by the token owner could be issued to any 3rd party the owner wishes to authorize to use the abstracted asset.
...
Applying this layered design approach to our car-key token example, we find that platforms like Google Wallet could potentially provide additional authorisation features useful to a car wash company. However, the real innovation lies in the hands of car vendors, who could enhance their car-key tokens with functionalities like a car wash mode or geo-fenced keys that restrict the car from being driven onto roads.
...
Secondly, the deterministic behavior of Smart Contract Tokens, governed by their embedded protocols, ensures consistent and predictable operation. This predictability is crucial for trust anchors, as web applications depending on them need to be confident that the tokens will behave as expected. Decentralised mechanism such as Decentralized Autonomous Organisations (DAOs)  enhanced the ability to update smart contracts while adhering to established trust protocols. 
...
Smart Contract function calls may bear a resemblance to the execution of RESTful APIs; facilitated as they are via Remote Procedure Call (RPC) interfaces, they lack a comprehensive service matrix. This includes aspects such as uptime, Service Level Objectives (SLOs), optimisation, load balancing, and confidentiality (ensuring that the RPC server does not leak information about executed calls). 
...
A Token Runtime Environment is a distinct operational architecture that enables the functionality and trusted behaviour of Smart Tokens. It serves as a secure and separate environment from the application it supports, providing the necessary interfaces, managing the token's state, and turns trust to the underlying token's core logic into a trusted token web-service. The Token Runtime Environment is crucial for Smart Tokens to serve as trust anchors, providing the necessary functions while maintaining a high level of security and trust.
